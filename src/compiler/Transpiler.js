...
import { createFile } from "./utils/FileOperations.js";
...
export class Transpiler {
     ...
     * @param {any} run
     */
    constructor(absolutePath, run) {
        this.CLRSFile = path.basename(absolutePath);
        const ext = path.extname(this.CLRSFile);

        this.JSFile = ext === ""
            ? this.CLRSFile + ".js"
            : this.CLRSFile.replace(ext, ".js");

        const tempKey = globalThis.ProgramKey;

        this.JSFile = run
            ? tempKey + "_" + this.JSFile
            : this.JSFile;

        this.JSDir = run
            ? path.join('.')
            : path.join(path.dirname(absolutePath), '.clrs', 'js');
    }
    ...
    async transpile(ast) {
        ...
        await createFile(fileContent, this.JSFile, this.JSDir);
    }
    ...
    readStatement(statement) {
        ...
        return identifiers
            .map((/** @type {any} */ identifier) => {
                ...
                return `
${expr} = String(await inputData_${globalThis.ProgramKey}());
